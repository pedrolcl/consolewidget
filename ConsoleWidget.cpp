...
ConsoleWidget::ConsoleWidget(QWidget *parent)
    ...
    , completer_(0)
{
    iodevice_ = new ConsoleIODevice(this, this);
...
void ConsoleWidget::setMode(ConsoleMode m)
{
    if (m==mode_) return;
...
void ConsoleWidget::updateCompleter()
{
...
void ConsoleWidget::checkCompletionTriggers(const QString &txt)
{
...
void ConsoleWidget::replaceCommandLine(const QString &str)
{
...
/////////////////// ConsoleWidget::History /////////////////////

#define HISTORY_FILE ".command_history.lst"

ConsoleWidget::History ConsoleWidget::history_;

ConsoleWidget::History::History(void)
    : pos_(0)
    , active_(false)
    , maxsize_(10000)
{
...
void ConsoleWidget::History::add(const QString &str)
{
    active_ = false;
...
void ConsoleWidget::History::activate(const QString &tk)
{
    active_ = true;
    token_ = tk;
    pos_ = -1;
...
bool ConsoleWidget::History::move(bool dir)
{
    if (active_)
    {
...
int ConsoleWidget::History::indexOf(bool dir, int from) const
{
    int i = from, to = from;
    if (dir)
    {
...
{
    ConsoleIODevice *d = qobject_cast<ConsoleIODevice *>(s.device());
...
QTextStream &inputMode(QTextStream &s)
{
    ConsoleIODevice *d = qobject_cast<ConsoleIODevice *>(s.device());
    if (d && d->widget())
        d->widget()->setMode(ConsoleWidget::Input);
    return s;
}
QTextStream &outChannel(QTextStream &s)
{
    ConsoleIODevice *d = qobject_cast<ConsoleIODevice *>(s.device());
    if (d)
        d->setCurrentWriteChannel(ConsoleWidget::StandardOutput);
    return s;
}
QTextStream &errChannel(QTextStream &s)
{
    ConsoleIODevice *d = qobject_cast<ConsoleIODevice *>(s.device());
    if (d)
        d->setCurrentWriteChannel(ConsoleWidget::StandardError);
    return s;
}
